...
onSelectCountry(): void {
  if (this.professionaldata && this.professionaldata.Address !== 'Tunisia') {
    this.professionaldata.Region = '';
...
getCities(country: string, region: string): string[] {
      ...
      this.prosevices.createPersonalData(professionalData).subscribe(
        (data) => {
          ...
          this.messageService.add({
            severity: "info",
            summary: "Confirmed",
            detail: "proffesional data created"
          });
          this.DataCharger()
        }
      )
  ...
  onSubmitUpdate() {
    if (this.newprofessionalDataForm.valid) {
      ...
      // Submit the form data to the backend or perform any other action
      console.log("Form value update:", JSON.stringify(this.newprofessionalDataForm.value, null, 2));
      this.prosevices.updateProfessionalDataById(UpdateprofessionalData).subscribe(
        (data) => {
          ...
          this.messageService.add({
            severity: "info",
            summary: "Confirmed",
            detail: "proffesional data Updated"
          });
          this.closeAllModals();
  ...
  initializeFormForUpdate() {
    if (!this.professionaldata) {
      console.error('professionaldata is null');
      return;
    }
  
    this.CreateFormForUPdate()
    this.newprofessionalDataForm.patchValue({
      ...
      Address: this.professionaldata.Address,
      Region: this.professionaldata.Region,
      City: this.professionaldata.City,
    });
